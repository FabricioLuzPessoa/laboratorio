<?php
	...
	/*
		-- Faço uma consulta na tabela exames_pendentes baseado no id_exame_pendente
		-- e crio o pdf usando o dompdf e armazeno no banco de dados
	*/
	...
	$consulta = "SELECT id_exame_pendente, documento, data_inclusao, id_paciente, nome_paciente, id_funcionario FROM 
														exames_pendentes WHERE id_exame_pendente='" . $_GET['id_exame_pendente'] . "'";
	...
	if($row != NULL) {
		...
		$id_exame_pendente = $row['id_exame_pendente'];  // id do exame.
		$documento         = $row['documento'];          // representar o array com as informações do exame.
		$data_inclusao     = $row['data_inclusao'];      // data de inclusão.
		$id_paciente       = $row['id_paciente'];        // id do paciente.
		$nome_paciente     = $row['nome_paciente'];      // nome do paciente.
	...
	if( isset($array['proteina_total']) ) {
		...
		$pagina_html .= '<p>
			...
			Relação A/G.....: '    . $array['relacao_a_g']    .'  g/dL. <br/><br/>
			<div class="fontemenor">	
				AMOSTRA: Soro.		             <br/>
	...
	$pagina_html .=	'</div>
			...
			<footer class="centralizar" id="assinatura" >
				<hr/>
	...
	$nome_documento = time().'-exame.pdf';
		
	// Salva o arquivo no servidor que consiste da data e do nome concatenado
	file_put_contents('exames/' . $nome_documento, $dompdf -> output());
		
	// String de que insere a informação no banco de dados.
	$consulta = "INSERT INTO exames (documento, data_inclusao, id_funcionario, id_paciente ) VALUES ('$nome_documento' , NOW(), '$id_funcionario', '$id_paciente' )";
		
	// executa o comnado da consulta.
	mysqli_query($dbc, $consulta) or die('Erro ao acessar o banco de dados!');
		
	//Após a publicação do exame, eu tenho que apagar o registro da tabela.
	$consulta_deletar_exame = "DELETE FROM exames_pendentes where id_exame_pendente = '$id_exame_pendente' ";
		
	// executa o comnado da consulta.
	mysqli_query($dbc, $consulta_deletar_exame) or die('Erro ao acessar o banco de dados!');
		
		
	$url_paciente = 'http://' . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . '/exames_pendentes.php';
